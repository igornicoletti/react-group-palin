        ...
        <div className="mt-16 flex justify-center gap-x-10">
          {navigation.social.map((item) => (
            <a key={item.name} href={item.href} className="text-dracula-white/75 hover:text-dracula-white" target='_blank'>
              ...
              <item.icon aria-hidden="true" weight="duotone" className="size-6 text-dracula-white/50 hover:text-dracula-main" />
            </a>
          ))}
        </div>
        <p className="mt-10 text-center text-sm/6 text-dracula-white/50">&copy; 2025 Palin & Martins. Todos os direitos reservados.</p>
      </div>
    </footer>
  )
}
